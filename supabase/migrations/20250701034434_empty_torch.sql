/*
  ...
  1. 変更内容
    ...
    - RLSポリシーを修正して管理者が全ての日記にアクセスできるようにする
...
-- 新しいポリシーを作成
CREATE POLICY "admin_panel_fix_policy" ON diary_entries
  FOR ALL 
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
COMMENT ON POLICY "admin_panel_fix_policy" ON diary_entries IS '認証済みユーザーが全ての日記エントリーにアクセスできるようにするポリシー';