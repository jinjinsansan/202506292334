...
CREATE OR REPLACE FUNCTION remove_duplicate_diary_entries() RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  WITH duplicates AS (
    SELECT 
      ...
      realization,
      ROW_NUMBER() OVER (
        PARTITION BY user_id, date, emotion, event, realization
...
-- 3. 同期時に重複チェックを行うためのトリガー関数
CREATE OR REPLACE FUNCTION prevent_diary_duplication() RETURNS TRIGGER AS $$
DECLARE
  existing_entry_id UUID;
BEGIN
  -- 同じuser_id, date, emotion, event, realizationを持つエントリーが既に存在するか確認
  ...
  WHERE 
    ...
    event = NEW.event AND
    realization = NEW.realization AND
    id != NEW.id
  LIMIT 1;
