...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'diary_entries' AND policyname = 'diary_entries_all_access_v5'
  ) THEN
    -- ポリシーが存在しない場合のみ作成
    EXECUTE 'CREATE POLICY "diary_entries_all_access_v5" ON diary_entries
      FOR ALL 
      TO authenticated
      USING (true)
      WITH CHECK (true)';
