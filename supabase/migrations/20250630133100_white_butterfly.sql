/*
  ...
  1. 新しいテーブル
    - `diary_delete_logs` - 日記削除操作のログを記録するテーブル
      - `id` (uuid, primary key)
      - `diary_id` (uuid) - 削除された日記のID
      - `user_id` (uuid) - 削除された日記のユーザーID
      - `deleted_at` (timestamp) - 削除日時
      - `deleted_by` (text) - 削除を実行したアカウント
      - `sync_status` (text) - 同期状態（pending, completed, failed）
      - `error_message` (text) - エラーメッセージ（失敗時）

  2. トリガー
    - 日記削除時に自動的にログを記録するトリガー
    - 古い削除ログを自動的にクリーンアップするトリガー
