...
BEGIN
  ...
  BEGIN
    ...
    EXECUTE 'DROP POLICY IF EXISTS "admin_panel_search_fix" ON diary_entries';
    EXECUTE 'DROP POLICY IF EXISTS "admin_panel_search_fix_final" ON diary_entries';
    EXECUTE 'DROP POLICY IF EXISTS "admin_panel_fix_policy" ON diary_entries';
  EXCEPTION
    WHEN OTHERS THEN
      -- エラーを無視して続行
      NULL;
...
-- 新しいポリシーを作成
CREATE POLICY "admin_panel_display_fix" ON diary_entries
  FOR ALL 
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
COMMENT ON POLICY "admin_panel_display_fix" ON diary_entries IS '認証済みユーザーが全ての日記エントリーにアクセスできるようにするポリシー';