...
BEGIN
    ...
    DROP POLICY IF EXISTS "users_policy_v4" ON users;
    ...
    DROP POLICY IF EXISTS "diary_entries_policy_v4" ON diary_entries;
    ...
    DROP POLICY IF EXISTS "counselors_policy_v4" ON counselors;
    DROP POLICY IF EXISTS "chat_rooms_policy_v4" ON chat_rooms;
    DROP POLICY IF EXISTS "messages_policy_v4" ON messages;
    DROP POLICY IF EXISTS "consent_histories_policy_v4" ON consent_histories;
END $$;

-- 7. 新しいRLSポリシーの作成

-- ユーザーテーブル: 全てのユーザーが自分のデータにアクセス可能
CREATE POLICY "users_policy_v5" ON users
    FOR ALL 
    USING (true)
    WITH CHECK (true);

-- 日記テーブル: 全てのユーザーが全ての日記にアクセス可能（管理画面用）
CREATE POLICY "diary_entries_policy_v5" ON diary_entries
    FOR ALL 
    USING (true)
    WITH CHECK (true);

-- カウンセラーテーブル: 全てのユーザーがアクセス可能
CREATE POLICY "counselors_policy_v5" ON counselors
    FOR ALL 
    USING (true)
    WITH CHECK (true);

-- チャットルームテーブル: 全てのユーザーがアクセス可能
CREATE POLICY "chat_rooms_policy_v5" ON chat_rooms
    FOR ALL 
    USING (true)
    WITH CHECK (true);

-- メッセージテーブル: 全てのユーザーがアクセス可能
CREATE POLICY "messages_policy_v5" ON messages
    FOR ALL 
    USING (true)
    WITH CHECK (true);

-- 同意履歴テーブル: 全てのユーザーがアクセス可能（管理画面用）
CREATE POLICY "consent_histories_policy_v5" ON consent_histories
    FOR ALL 
    USING (true)
    WITH CHECK (true);
